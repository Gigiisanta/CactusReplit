...
from cactus_wealth.schemas import BacktestRequest, BacktestResponse, PortfolioComposition
...
class TestPortfolioBacktestService:
    ...
    async def test_portfolio_backtest_success(
        ...
        with patch.object(
            backtest_service, 
            '_download_historical_data_cached', 
            ...
            with patch.object(
                ...
                mock_dividends.return_value = {"SPY": pd.Series(dtype=float), "AAPL": pd.Series(dtype=float)}
                
                # Execute backtesting
                result = await backtest_service.perform_backtest(sample_backtest_request)
    ...
    async def test_cache_serialization_robustness(self, backtest_service):
        ...
        test_series = pd.Series([100, 101, 102], index=pd.date_range('2023-01-01', periods=3))
        
        # Test with DataFrame (edge case that was causing the error)
        test_dataframe = pd.DataFrame({'Close': [100, 101, 102]}, index=pd.date_range('2023-01-01', periods=3))
        ...
        with patch('yfinance.download') as mock_yf:
            ...
            try:
                ...
                result = await backtest_service._download_historical_data_cached(["SPY"], "1mo")
    ...
    def test_ensure_timezone_aware_utility_function(self, backtest_service):
        ...
        tz_aware_index = pd.DatetimeIndex(['2023-01-01', '2023-01-02'], tz='America/New_York')
    ...
    async def test_get_dividends_handles_timezone_differences(self, backtest_service):
        ...
        dividend_dates = pd.date_range('2023-01-01', '2023-12-31', freq='QS', tz='America/New_York')
        ...
        with patch('yfinance.Ticker') as mock_yf_ticker:
            ...
            try:
                result = await backtest_service._download_dividend_data_concurrent(['SPY'], '6mo')
