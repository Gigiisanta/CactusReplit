...
from cactus_wealth.models import User, Client, Portfolio, Position, Asset, Report, PortfolioSnapshot, UserRole, RiskProfile, AssetType
...
def test_clients_and_portfolios_fixture(session: Session, admin_user: User, advisor_user: User, test_assets: list[Asset]):
    ...
    portfolios_data = [
        # Admin client 1 portfolio
        {
            ...
            "positions": [
                {"asset": test_assets[0], "quantity": 100, "purchase_price": 150.0},  # AAPL
                {"asset": test_assets[2], "quantity": 50, "purchase_price": 400.0},   # SPY
            ]
        },
        # Admin client 2 portfolio
        {
            ...
            "positions": [
                {"asset": test_assets[1], "quantity": 25, "purchase_price": 2000.0},  # GOOGL
            ]
        },
        # Advisor client portfolio
        {
            ...
            "positions": [
                {"asset": test_assets[0], "quantity": 50, "purchase_price": 160.0},   # AAPL
                {"asset": test_assets[2], "quantity": 25, "purchase_price": 410.0},  # SPY
            ]
        }
    ]
    ...
    for portfolio_data in portfolios_data:
        portfolio = Portfolio(
            ...
            client_id=portfolio_data["client"].id
        )
...
def test_dashboard_summary_unauthorized(client: TestClient):
...
def test_dashboard_summary_invalid_token(client: TestClient):
...
def test_dashboard_summary_response_schema(
    ...
    assert isinstance(data["assets_under_management"], (int, float))
    assert data["monthly_growth_percentage"] is None or isinstance(data["monthly_growth_percentage"], (int, float))
...
def test_dashboard_reports_kpi_with_generated_reports(
    ...
    admin_response = client.get("/api/v1/dashboard/summary", headers=admin_headers)
    ...
    advisor_response = client.get("/api/v1/dashboard/summary", headers=advisor_headers)
...
def test_dashboard_reports_kpi_with_old_reports(
    ...
    old_report_date = datetime.now(timezone.utc) - timedelta(days=100)
    old_report = Report(
...
def test_dashboard_monthly_growth_with_snapshots(
    ...
    assert abs(data["monthly_growth_percentage"] - 0.0467) < 0.001  # Allow small rounding differences
    
    print(f"Monthly Growth: {data['monthly_growth_percentage']:.4f} ({data['monthly_growth_percentage']*100:.2f}%)")
