...
from cactus_wealth.models import User, Report
from cactus_wealth.core.dataprovider import get_market_data_provider, MarketDataProvider
...
@router.post("/clients/{client_id}/generate-report", response_model=schemas.ReportResponse)
async def generate_client_report(
    ...
    """
    logger.info(f"Report generation requested for client {client_id} by user {current_user.id}")
...
async def download_report(
    ...
    try:
        ...
        if not report:
            logger.warning(f"Report {report_id} not found or access denied for user {current_user.id}")
            raise HTTPException(
