...
from cactus_wealth.schemas import InsurancePolicyCreate, InsurancePolicyRead, InsurancePolicyUpdate
...
def get_policy_service(session: Session = Depends(get_session)) -> InsurancePolicyService:
    """Dependency to get insurance policy service."""
    return InsurancePolicyService(db_session=session)


@router.post("/clients/{client_id}/insurance-policies/", response_model=InsurancePolicyRead, status_code=status.HTTP_201_CREATED)
def create_insurance_policy_for_client(
    ...
    policy = policy_service.create_policy_for_client(
        policy_data=policy_create,
        client_id=client_id,
        current_advisor=current_user
    )
...
@router.get("/clients/{client_id}/insurance-policies/", response_model=List[InsurancePolicyRead])
