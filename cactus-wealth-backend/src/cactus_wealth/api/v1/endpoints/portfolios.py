...
def get_portfolio_valuation(
    ...
    """
    logger.info(
        f"Portfolio valuation requested by user {current_user.email} for portfolio {portfolio_id}"
    )
...
def download_portfolio_report(
    ...
    """
    logger.info(
        f"PDF report download requested by user {current_user.email} for portfolio {portfolio_id}"
    )
    ...
    try:
        ...
        pdf_bytes = report_service.generate_portfolio_report_pdf(
            valuation_data=valuation, portfolio_name=portfolio.name
        )
...
async def backtest_portfolio(
    ...
    """
    logger.info(
        f"Optimized portfolio backtesting requested by user {current_user.email}"
    )
