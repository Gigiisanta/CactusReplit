...
export function InsurancePoliciesSection({ clientId, policies, onDataChange }: InsurancePoliciesSectionProps) {
  ...
  const handleEditPolicy = async (policyId: number, policyData: {
    policy_number?: string;
    insurance_type?: string;
    premium_amount?: number;
    coverage_amount?: number;
    ...
    toast.promise(updatePromise, {
      ...
      error: (err) => {
        const errorMessage = err instanceof Error ? err.message : 'No se pudo actualizar la póliza';
  ...
  const handleDeletePolicy = async (policyId: number) => {
    if (!confirm('¿Estás seguro de que quieres eliminar esta póliza de seguro?')) {
      return;
    }
    ...
    try {
      ...
      toast.promise(deletePromise, {
        ...
        error: (err) => {
          const errorMessage = err instanceof Error ? err.message : 'No se pudo eliminar la póliza';
          return `❌ Error: ${errorMessage}`;
        },
      });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              ...
              Cobertura Total: {formatCurrency(totalCoverage)} • 
